...
class ExpensesViewModel{
    func AddExpense(expense : Expense) -> Result{
            
            var result = Result()
            let context = DB.init()
            let query = "Insert INTO Balance(nombre, cantidad, idTipo, idSubcategoria, idUser, fecha) VALUES(?,?,?,?,?,?)"
    ...
    func GetAll() -> Result{
            var result = Result()
            let context = DB.init()
            let query = "SELECT idBalance, nombre, cantidad, idTipo, idSubcategoria, idUser, fecha FROM Balance ORDER BY idBalance DESC"
            var statement : OpaquePointer? = nil
            do{
                if try sqlite3_prepare_v2(context.db, query, -1, &statement, nil) == SQLITE_OK{
                    result.Objects = []
    ...
    func GetSubcategory(idSubcategory : Int)->Result{
            var result = Result()
            let context = DB.init()
            let query = "SELECT idBalance, nombre, cantidad, idTipo, idSubcategoria, idUser, fecha FROM Balance WHERE ((idSubcategoria = \(idSubcategory)))"
            var statement : OpaquePointer? = nil
            do{
                if try sqlite3_prepare_v2(context.db, query, -1, &statement, nil) == SQLITE_OK{
                    result.Objects = []
        ...
        func GetBalanceCategoria(idSubcategoria : Int, subcategoria : String)->Result{
            var Expense = 0
            var Income = 0
            var idtipo = 0
            var result = Result()
